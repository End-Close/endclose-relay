...
application is built from (`@end-close/relay`, `packages/core/src/mask/defaults.ts`). The
...
surface (records, admin API, dashboards) can return them. Provide them however you
already manage container secrets (Compose `env_file`, cloud secret managers, systemd
credentials, …). Optionally set `RELAY_SECRETS_FILE` to a host-mounted dotenv path:
values fill unset or blank env vars at boot (names, never values, are logged), so
secrets can live only on the host if that is your policy.
