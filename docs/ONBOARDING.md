...
- Egress allowed to `api.endclose.com:443` and your image registry. Nothing else is
  needed (see `docs/SECURITY.md`). The relay asks that same API whether End Close manages
  its configuration and reports its instance manifest (boot, heartbeat, shutdown); set
  `RELAY_REMOTE_CONFIG=off` / `RELAY_TELEMETRY=off` if you do not want either.
...
If End Close manages your environment's configuration (the API key is environment-scoped,
so sandbox and production keys each follow their own), the relay comes up running End
Close's document and keeps it current — review it in the config tab before pointing
Payabli at the relay; edits are made in End Close. Otherwise, with a seed file the relay
comes up configured from it; without one it boots into **bootstrap mode** (no webhooks
accepted yet). `RELAY_REMOTE_CONFIG=off` keeps configuration local regardless. Open the admin UI — `ssh -L 8081:127.0.0.1:8081 <host>` then
...
For a managed container deploy with EFS-backed storage, see
[`deploy/fargate/`](../deploy/fargate/). Same bootstrap/admin flow once the task is up;
reach `:8081` over a private path (VPN / Tailscale exit node into the VPC), not the
public ALB.
...
**Upgrades:** bump the image tag and recreate against the **same data volume** so
